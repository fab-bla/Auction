...
* Can machine learning methods be used to provide an improvement over the engineer's estimate in predicting the award price of auctions held by the CDOT?

  + This model would enable the auctioning entity to plan their budget more accurately.
  
* Unsupervised Collusion Detection

  + Does the interaction of certain firms lead to a systematic increase in the award price of auctions?
...
# Regularized Regression: [Intuition](https://www.desmos.com/calculator/59axr4plef)
...
```{r, out.width = "90%", fig.align = "center", fig.cap = "Regularization Utilizing Different Metrics (Toth 2022)"}
# graph
...
```{r, out.width = "70%", fig.align = "center", fig.cap = "Bias Variance Tradeoff (Fortmann-Roe 2012)"}
# graph
...
* Why is it problematic if we use the Data to choose the variables in our model?
  - A pre-selection that minimizes some predictive error, will choose variables that have a relationship with the dependent variable  $\implies$ Type I error rate increases!
...
If you are interested to learn more about post-selection inference. Prof. Loftus held a very insightful and easy to understand presentation which is available on youtube: \textcolor{teal}{\textit{youtube.com/watch?v=bQhEALoxoGE}}.

The link to the interactive Elastic-Net example may be found here: \textcolor{teal}{\textit{desmos.com/calculator/skbksmodhd}}.
