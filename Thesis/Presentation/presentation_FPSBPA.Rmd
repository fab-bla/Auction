...
# Methods: Elastic Net
Given the model matrix $\mathbf{X} \in \mathbb{R}^{n\times p}$ we may formulate the elastic net as a linear model that utilizes $\ell_1$ and $\ell_2$ regularization.Further, suppose that $\alpha \in [0, 1]$ and $t \in \mathbb{R}^+$, we can then find the elastic net estimator as a constrained minimization problem, 
...
The best performing regularized model is a lasso regression model. The lasso penalty shrinks some elements of the parameter vector $\hat\beta$ exactly to zero. 
