...
\]
 ...
 Additionally, to gain some intuition for the candidate variable sampling we may benefit from examining $B$ random variables $T_b$, that are identically distributed but not necessarily independent. Let the variance of each random variable $T_b$ be represented by $\sigma^2$ and further let the positive pairwise correlation be represented by $\rho$. Accordingly, the prediction that we obtain by the average over all trees in the forest exhibits the variance: 
 \[
 \mathbb{V}ar(\hat f_{rf}) = \mathbb{V}ar \left[ \frac{1}{B} \mathlarger{\sum}\limits_{b = 1}^B T_B(x) \right] = \frac{1}{B^2} \mathbb{V}ar \left[ \mathlarger{\sum}\limits_{b = 1}^B T_B(x) \right] = \frac{1}{B^2} \mathlarger{\sum}\limits_{i = 1}^B \mathlarger{\sum}\limits_{j = 1}^B \mathbb{C}ov \left[T_i, T_j \right].
 \]
Now, given that in the double sum there are $B^2$ summands and we know that B of those are just $\mathbb{C}ov\left[T_i, T_i \right] = \mathbb{V}ar\left[T_i\right] = \sigma^2$, we are left with $B^2 - B$ terms, and thus we find,
\[
\frac{1}{B^2} \mathlarger{\sum}\limits_{i = 1}^B \mathlarger{\sum}\limits_{j = 1}^B \mathbb{C}ov \left[T_i, T_j \right] = \frac{1}{B^2} \left[B\sigma^2 + (B^2 - B)\rho\sigma^2 \right] = \frac{\sigma^2}{B} +  \frac{B - 1}{B}\rho\sigma^2.
\]
Some trivial rearrangements then lead us to the more easily interpretable result,
\[
\mathbb{V}ar(\hat f_{rf}) = \rho\sigma^2 + \frac{\sigma^2}{B}(1 - \rho).
\]
This result, simply demonstrates the idea behind the variance reduction achieved via bagging and candidate variable sampling. We observe that the second term approaches zero as the number of trees in the forest approach infinity. Additionally, we also observe that the first term depends on the correlation of the individual trees, thus by choosing different candidate variables at each split in different trees we aim to reduce the correlation of the individual trees in our forest.
