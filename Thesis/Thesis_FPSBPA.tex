...
{\centering
	\begin{minipage}{.9\linewidth}
		\begin{algorithm}[H]
			...
			\KwData{A training dataset $\mathcal{D}\{\{\bm x_i, y_i\}\}$, with $i = 1, ..., n$, $x_i \in \mathbb{R}^d$ and $y_i \in \mathbb{R}$. A $n \times f$ tuning grid for the random forest hyperparameters, in this case this tuning grid also contains the parameters for recursive feature elimination $p$ and $k$, $K$ cross validation folds and a loss function $l(y_i, \hat y_i)$ to determine the CV error.}
			\begin{enumerate}
				\item For all $K$ cross validation folds, repeat:
				\begin{enumerate}
					\item For all $n$ rows in the RF tuning grid, repeat:
