...
{\centering
	\begin{minipage}{.9\linewidth}
		\begin{algorithm}[H]
			...
			\KwData{A training dataset $\mathcal{D}\{\{\bm x_i, y_i\}\}$, with $i = 1, ..., n$, $x_i \in \mathbb{R}^p$ and $y_i \in \mathbb{R}$. The structure complexity parameter $\gamma$, the regularization parameter $\lambda$, the learning rate $\eta$, the maximum depth $d_{max}$ and the number of trees to be fit in succession $n_{iter}$.}
			\begin{enumerate}
...
{\centering
	\begin{minipage}{.9\linewidth}
		\begin{algorithm}[H]
			...
			\KwData{A training dataset $\mathcal{D}\{\{\bm x_i, y_i\}\}$, with $i = 1, ..., n$, $x_i \in \mathbb{R}^p$ and $y_i \in \mathbb{R}$. A $n \times k$ tuning grid for the random forest hyperparameters, a $l \times p$ tuning grid for the logistic PCA hyperparameters, $K$ cross-validation folds and a loss function $l(y_i, \hat y_i)$ to determine the CV error.}
			\begin{enumerate}
