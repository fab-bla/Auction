...
Where 
\[
g_i = \frac{d l(y_i, \hat y^{(t-1)})}{d \hat y_i^{(t-1)}} \text{ and } h_i = \frac{d^2 l(y_i, \hat y^{(t-1)})}{d \hat y_i^{(t-1)}},
\]
...
\[
\mathcal{L}_{opt} = \frac{1}{2}\frac{RSS_{i \in I_j}}{|I_j| + \lambda} \approx \frac{RSS_{i \in I_j}}{|I_j| + \lambda},
\]
given that the coefficient is constant for all evaluated splits, it may be omitted. Now that we have a tool for calculating the optimal output weight $w_j$ as well as a procedure to evaluate the quality of a split, we can formulate XGboost as an algorithm:

{\centering
	\begin{minipage}{.9\linewidth}
		\begin{algorithm}[H]
			\caption{\textit{XGboost}}\label{alg:one}
			\KwData{}
			\begin{enumerate}
				\item 
				\begin{enumerate}[i.]
