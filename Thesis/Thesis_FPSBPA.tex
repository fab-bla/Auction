...
The idea of simple k-fold cross validation for hyper-parameter optimization is to split the training data into $K = 5$, equally-sized parts. Then suppose the indexing function $\kappa: \{1, ..., n\} \rightarrow \{1, ..., K\}$, it indicates which observation is $i$ out of $n$ is allocated to the $K - 1$ training folds and the K'th validation fold. Further, let $\hat f^{-k}(x)$, the fitted predictive function computed with the k'th part of the data removed and let $\bm \alpha$, represent a vector of hyper-parameters used to fit $\hat f^{-k}(x, \alpha)$ \citep{hastie09}. We can then represent the cross validation error as,
\begin{gather*}
	CV(\hat f) = \frac{1}{K |\kappa|} \mathlarger{\sum}\limits_{k = 1}^{K} \left[ \mathlarger{\sum}\limits_{i = 1}^{|\kappa|} L(y_i, \hat f^{-\kappa}(x, \bm \alpha)) \right].
\end{gather*}
Where $L$ represents some loss function, e.g., the root mean square error or the mean absolute error.\\
Now given that some pre-processing schedules like Logistic PCA or feature selection have an effect on the choice of the optimal hyper-parameter vector $\bm \alpha$, within the $K$ folds there a secondary CV process is established. Consider PCA as an example, the optimal number of principle component is a parameter to be chosen via cross validation, further we cannot apply our predictive function $\hat f^{-k}(x)$ on the validation fold without previously using the training data to determine the loading vectors used to apply the data set transformation. Accordingly, the idea of nested cross validation is to assess the optimal value of principle components in an inner loop, then use the acquired loading vectors to apply the data transformation on the training data to then run the outer loop over the hyper-parameter set $\bm \alpha$. Note, that it considered a severe data leak to just apply PCA to the validation set directly.
