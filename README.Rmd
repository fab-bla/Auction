...
* Elastic Net
* Linear Model
  - [with logistic PCA preprocessing](https://github.com/Base-R-Best-R/Auction/blob/main/Code/Models/Colab/CV_PreProcess_LM.ipynb)
* Random Forest
...
per_LM <- cbind(per_LM, "LM") |> data.frame()

# lst
lst <- list(per_lasso, per_RF, per_xgb, per_LM)
...
dat_per_plot$Model <- factor(dat_per_plot$Model, 
                             levels = c("Act.", "Lasso Reg.", "LM","RF", "XGB","Eng. Est."))
...
Map(\(Pred, title, bool, fill){
  ...
  Act_vs_Pred(Act, Pred, 
            title = title, bg_alt = bool, size = 2.5, shape = 21, alpha = 0.6,
            outcol = "black", fill = fill) 
  
  
}, AWPs, c("Lasso Regression", "LM with logPCA Preprocessing", 
           "RF with logPCA Preprocessing", "eXtreme Gradient Boosting",
           "Engineer's Estimate"), c(T, F, F, F, F), 
          c("darkgoldenrod1", "firebrick", "forestgreen",
                "darkmagenta", "wheat3")) -> plots 
...
}) |> knitr::kable(col.names = c("Lasso", "LM", "RF", "XGB", "Eng. Est."))
```
